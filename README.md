...
### 2026/02/11 — ゲームプレイ改善アップデート

- **レトロ風タイトル画面**: タイトルを「SUPER RUNNER」に変更、Press Start 2P ピクセルフォント、星空アニメーション、CRT スキャンライン演出を追加
- **ニックネーム & ランキングシステム**: ゲーム開始時にニックネームを入力、ボス撃破後に localStorage でスコアランキング（TOP 10）を表示
- **ブロックバンプ**: 下から叩いたブロックがバンプし、上に乗っている敵を跳ね飛ばして倒す
- **月アイテムの落下方向**: ブロックから出現した月アイテムがランダムに左右に飛び出して落下する
- **パイプワープ（地下ボーナスエリア）**: World 1-1 / 1-2 の最初のパイプに下キーで入ると地下コインエリアへワープ。出口パイプで地上に帰還
- **パイプ暗転トランジション**: パイプ出入り時にプレイヤーがパイプに沈む→画面暗転→ワープ→画面復帰→パイプからせり上がるアニメーション
- **パイプ前面描画**: パイプは常にプレイヤーより前面に描画され、出入り時に自然な見た目を実現
- **下キー入力改善**: 下キーを押した瞬間のみパイプに入れるよう変更（押し続けによる誤入力防止）
